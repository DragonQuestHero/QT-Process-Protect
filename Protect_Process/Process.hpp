...
namespace CG
{
	...
	public:
		...
		bool Get_Process_Image(HANDLE Process_Handle,UNICODE_STRING *Process_Path)
		{
			...
			Process_Path = new UNICODE_STRING();
	...
	private:
		_ZwQueryInformationProcess ZwQueryInformationProcess = nullptr;
		_PsGetProcessPeb PsGetProcessPeb = nullptr;
	private:
		bool Get_NtQuery_Func()
		{
			NTSTATUS status = 0;
			UNICODE_STRING ZwQueryInformationProcess_Func_Name;
			RtlInitUnicodeString(&ZwQueryInformationProcess_Func_Name, L"ZwQueryInformationProcess");
			ZwQueryInformationProcess = (_ZwQueryInformationProcess)
				MmGetSystemRoutineAddress(&ZwQueryInformationProcess_Func_Name);
			if (!ZwQueryInformationProcess)
			{
				KdPrint(("Get _ZwQueryInformationProcess Error\n"));
				return false;
			}

			UNICODE_STRING Func_Name;
			RtlInitUnicodeString(&Func_Name, L"PsGetProcessPeb");
			_PsGetProcessPeb PsGetProcessPeb = (_PsGetProcessPeb)
				MmGetSystemRoutineAddress(&Func_Name);
			if (!PsGetProcessPeb)
			{
				KdPrint(("Get Func_PsGetProcessPeb Error\n"));
				return false;
			}
